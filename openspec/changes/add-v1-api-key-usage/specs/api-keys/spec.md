...
- `daily_usage` containing exactly 30 UTC calendar-day rows, ordered oldest to newest, each with `date`, non-cached `tokens`, and `cost_usd`
...
- **AND** `daily_usage` returns 30 rows with zero `tokens` and zero `cost_usd`
